...
--
-- MIGRATION (if you already have v1 running):
--   alter table stats rename column strength to health;
--   alter table stats rename column ambition to work;
--   alter table stats add column if not exists relationships integer not null default 25 check (relationships between 0 and 100);
...
--   alter table quests add constraint quests_category_check check (category in ('health','mind','work','finance','relationships'));
--   update quests set category = 'work' where category = 'career';
--   (then create the reflections table below)
...
create table if not exists activity_log (
  id          uuid primary key default uuid_generate_v4(),
  user_id     uuid references auth.users(id) on delete cascade not null,
  entry_type  text not null check (entry_type in ('quest_complete','reward_redeemed','level_up','stat_boost','reflection')),
